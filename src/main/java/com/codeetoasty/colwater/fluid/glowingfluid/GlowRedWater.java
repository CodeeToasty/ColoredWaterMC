...
public abstract class GlowRedWater extends DyableWater {
    @Override
    public Fluid getStill() {
        return ModFluids.GLOW_STILL_RED;
    }

    @Override
    public Fluid getFlowing() {
        return ModFluids.GLOW_FLOWING_RED;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.GLOW_RED_BUCKET;
    }

    @Override
    protected BlockState toBlockState(FluidState fluidState) {
        // getBlockStateLevel converts the LEVEL_1_8 of the fluid state to the LEVEL_15 the fluid block uses
        return ModBlocks.GLOW_RED.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(fluidState));
    }
    ...
    public void randomDisplayTick(World world, BlockPos pos, FluidState state, Random random) {
        ...
        if (world.getBlockState(blockPos).isAir() && !world.getBlockState(blockPos).isOpaqueFullCube(world, blockPos)) {
            if (random.nextInt(25) == 0) {
                ...
                world.addParticle(ModParticles.RED_SUS, d, e, f, 0.0D, 0.0D, 0.0D);
            }
        }
    }
    public static class Flowing extends GlowRedWater {
        @Override
    ...
    public static class Still extends GlowRedWater {
        @Override
        protected boolean isInfinite(World world) {
            return false;
        }
